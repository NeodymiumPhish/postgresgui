...
struct ConnectionFormView: View {
    ...
    private var currentName: String {
        if inputMode == .individual {
            return showIndividualNameField && !individualName.isEmpty
                ? individualName
                : autoGeneratedIndividualName
        } else {
            return showConnectionStringNameField && !connectionStringName.isEmpty
                ? connectionStringName
                : autoGeneratedConnectionStringName
        }
    }

    private var autoGeneratedIndividualName: String {
        let hostValue = host.isEmpty ? "localhost" : host
        let portValue = port.isEmpty ? "5432" : port
        let usernameValue = username.isEmpty ? "postgres" : username
        let databaseValue = database.isEmpty ? "postgres" : database
        return "\(usernameValue)@\(hostValue):\(portValue)/\(databaseValue)"
    }

    private var autoGeneratedConnectionStringName: String {
        guard !connectionString.isEmpty,
              let parsed = try? ConnectionStringParser.parse(connectionString) else {
            return "New Connection"
        }
        let username = parsed.username ?? "postgres"
        let database = parsed.database ?? "postgres"
        return "\(username)@\(parsed.host):\(parsed.port)/\(database)"
    }
